		...
		public:
			...
			*/
			...
			virtual void setFixAxis(double x0,double y0,double z0,double x1,double y1,double z1)override
			{
				...
				v.targetY = 0.5*(y0+y1);
				v.targetZ = 0.5*(z0+z1);
				v.fix_xwidth = std::abs(x1-x0);
				v.fix_ywidth = std::abs(y1-y0);
				v.fix_zwidth = std::abs(z1-z0);
				v.viewType = PLComView::FixedAxis;
				v.target = m_id;
				m_view->send(&v);
			}
